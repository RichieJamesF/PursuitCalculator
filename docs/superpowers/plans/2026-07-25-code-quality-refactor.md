...
- **Environment note (added during implementation, 2026-07-25):** Docker cannot run on this machine — Docker Desktop's Linux engine needs WSL2, which isn't installed, and installing it needs Administrator rights this session doesn't have (confirmed: `docker info`, `docker ps`, `wsl --status` all fail). Task 4's harness (`docker-compose.yml`, `scripts/test-integration.mjs`, `tests/integration/helpers.mjs`) was written and statically reviewed but has never actually run against a live Postgres. Tasks 5-7 (integration tests for events/riders/groups routes) are deferred until Docker is available. For every later task whose "Run the full test suite" step says `npm run test:all`, substitute `npm test` (unit only) + `node --check <file>` for any modified/new file instead — do not attempt `npm run test:integration` or `npm run test:all`, they will fail for this same environment reason, not a code defect.
