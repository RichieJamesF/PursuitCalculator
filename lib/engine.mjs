...
// Effective sustainable power for a rider at the group's chosen effort level.
const powerOf = (r, p) => Number(r.ftp) * (p.effort / 100);
...
export function computeSheet(groups, ridersById, segments, p) {
  const list = groups.map((g) => {
    ...
    return { gid: g.id, locked: !!g.locked, wkg, dur,
      members: members.map((r, i) => ({ id: r.id, name: r.name, front: fronts[i] })),
